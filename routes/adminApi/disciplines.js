...
function filteredSearch(queryParams, filterFields, res, findQuery, countQuery) {
  filterFields.forEach(filter => {
    ...
    console.error(err);    res.status(500).send("DB paggination error");
  });
}
function getItem(req, res, next) {
  const id = req.params.id;
  console.log("opa-chirik");   model.findById({
    _id: id
  }).then(function(doc) {
    res.status(200).json(doc);
  }).catch(function(err) {
    console.error(err);    res.status(500).send("DB receiving entry by id error");
  });
}
function postItem(req, res, next) {
  ...
  newItem.save().then(function(doc) {
    ...
    console.error(err);    res.status(500).send("DB saving error");
  });
}
...
function deleteItem(req, res, next) {
  ...
  model.findByIdAndRemove(id).then(function(offer) {
    ...
    console.error(err);    res.status(500).send("DB deleting error");
  });
}
